...
import { rigChassis, rigChassisIdle, rigTracks, rigTool, rigLamp, rigCargo, rigVacuum, chargeSprite, chargeArmed, chargePreview, toolOverheat } from "./art-rig";
import { THREAT_DRAWERS, threatElite, threatTelegraph, resourceIcon, relicIcon, uiIcon, blueprintIcon } from "./art-entities";
import { buildProps, buildBase, buildVFX, buildInteract, logo, logoMono, favicon } from "./art-world";
...
/** PNG size budget (bytes): CI fails if any sheet exceeds it. */
const SHEET_BUDGET = 900 * 1024;
...
function main() {
  ...
  const manifest: {
    ...
    frames: Record<string, { sheet: string; x: number; y: number; w: number; h: number; tags?: string[] }>;
    animations: Record<string, { sheet: string; frames: string[]; fps: number; loop: boolean }>;
  } = {
    version: 2,
    generated: "deterministic",
    generator: "tools/gen-assets.ts",
    license: "original — generated by project tools, see docs/LICENSES.md",
    sheets: {},
    frames: {},
    animations: {},
  };
  ...
  rig.add("chassis_3", rigChassis(3)); // critical damage state
  rig.add("chassis_idle", rigChassisIdle());
  ...
  rig.add("vacuum_3", rigVacuum(18)); // magnet-streak wide ring
  rig.add("charge", chargeSprite());
  rig.add("charge_armed", chargeArmed());
  rig.add("charge_preview", chargePreview());
  rig.add("tool_overheat", toolOverheat());
  ...
  anim("rig", "charge_blink", ["charge", "charge_armed"], 4, true);
  ...
  for (const [family, draw] of Object.entries(THREAT_DRAWERS)) {
    ...
    enemies.add(`${family}_elite`, threatElite(family));
    enemies.add(`${family}_telegraph`, threatTelegraph(family));
  }
  ...
  const UI_KEYS = ["money", "cargo", "heart", "scanner", "depth", "marker", "charge", "core", "map", "lift", "temp", "press", "shield", "drill", "skull", "star",
    "filter", "zoom_in", "zoom_out", "export", "warn", "check", "lock", "bolt", "magnet", "skull2", "compass", "crate"];
  ...
  buildBase(reg("props"));
  buildInteract(reg("props"));
  buildVFX(reg("vfx"));
  anim("vfx", "scan_ping", ["vfx_scan_0", "vfx_scan_1"], 6, false);
  anim("vfx", "reswave", ["vfx_reswave_0", "vfx_reswave_1"], 8, false);
  anim("vfx", "explosion_full", ["vfx_ignite", "vfx_explosion_0", "vfx_explosion_1", "vfx_explosion_2", "vfx_explosion_3"], 14, false);

  // ---- standalone images ----------------------------------------------------
  fs.mkdirSync(PUB, { recursive: true });
  const logoPng = logo().toPNG();
  const logoMonoPng = logoMono().toPNG();
  const faviconPng = favicon().toPNG();
  if (!CHECK) {
    fs.writeFileSync(path.join(PUB, "logo.png"), logoPng);
    fs.writeFileSync(path.join(PUB, "logo-mono.png"), logoMonoPng);
    ...
    fs.writeFileSync(path.join(outDir, "logo-mono.png"), logoMonoPng);
  ...
  for (const sheet of Object.values(sheets)) {
    for (const [name, f] of sheet.frames) {
      // validation: duplicate frame names + out-of-bounds rects fail loudly
      if (seen.has(name)) {
        console.error(`duplicate frame name: ${name}`);
        process.exit(1);
      }
      seen.add(name);
      const size = manifest.sheets[sheet.name].size;
      if (f.x + f.w > size[0] || f.y + f.h > size[1]) {
        console.error(`frame out of bounds: ${name} in ${sheet.name}`);
        process.exit(1);
      }
      const tags: string[] = [];
      if (name.startsWith("t_")) tags.push("terrain");
      if (name.startsWith("ore_")) tags.push("ore");
      if (name.startsWith("tool_") || name.startsWith("chassis") || name.startsWith("tracks")) tags.push("rig");
      if (name.startsWith("vfx_")) tags.push("vfx");
      if (name.startsWith("prop_") || name.startsWith("base_")) tags.push("props");
      if (name.startsWith("res_") || name.startsWith("relic_") || name.startsWith("ui_")) tags.push("icons");
      if (name.includes("elite") || name.includes("telegraph")) tags.push("elite");
      manifest.frames[name] = { sheet: sheet.name, x: f.x, y: f.y, w: f.w, h: f.h, tags };
    }
  }
  const manifestPath = CHECK ? path.join(outDir, "asset-manifest.json") : path.join(PUB, "asset-manifest.json");
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 1));

  const fileCount = Object.keys(manifest.frames).length + 3;
  console.log(`generated ${Object.keys(sheets).length + 3} sheets, ${Object.keys(manifest.frames).length} frames, ${Object.keys(manifest.animations).length} animations → ${outDir}`);

  if (STATS || CHECK) {
    // size report + budget guard
    let over = 0;
    for (const f of fs.readdirSync(outDir)) {
      if (!f.endsWith(".png")) continue;
      const sz = fs.statSync(path.join(outDir, f)).size;
      console.log(`  ${f}: ${(sz / 1024).toFixed(1)} KB${sz > SHEET_BUDGET ? " OVER BUDGET" : ""}`);
      if (sz > SHEET_BUDGET) over++;
    }
    if (over > 0) {
      console.error(`asset budget FAILED: ${over} sheets exceed ${(SHEET_BUDGET / 1024).toFixed(0)} KB.`);
